#!/usr/bin/env python3
"""Report whether required GitHub Actions secrets exist without reading or exposing their values."""
...
DEFAULT_REPORT = Path("edge-github-secret-readiness.md")
SECRET_ARGUMENTS = (
    ("SUPABASE_ACCESS_TOKEN", "has_supabase_access_token"),
    ("SUPABASE_PROJECT_ID", "has_supabase_project_id"),
...
def render_report(rows: list[dict[str, str]], findings: list[str]) -> str:
    ...
    lines = [
        "# GitHub secret readiness for Edge deploy",
        "",
    ...
    lines.extend([
        ...
        "It never reads, prints, hashes or stores secret values.",
        "",
    ])
    return "\n".join(lines)
...
def run_check(*, raw_flags: dict[str, str], report_path: Path) -> int:
    ...
    if findings:
        print("GitHub secret readiness findings:")
        for finding in findings:
            print(f"  - {finding}")
        return 1
    print("GitHub secret readiness passed")
    return 0
...
def self_test() -> int:
    ...
    if findings or len(rows) != len(SECRET_ARGUMENTS):
        failures.append("complete GitHub secret configuration was rejected")

    missing = dict(all_present)
    missing["has_healthcheck_token"] = False
    _, missing_findings = evaluate(missing)
    if not any("PARKET_HEALTHCHECK_TOKEN" in finding for finding in missing_findings):
        failures.append("missing healthcheck token was not detected")
    ...
    for forbidden in ("token-value", "digest", "hash:"):
        if forbidden in report:
            failures.append(f"report contains forbidden value marker: {forbidden}")

    if failures:
        print("GitHub secret readiness self-test failed:")
        for failure in failures:
            print(f"  - {failure}")
        return 1
    print("GitHub secret readiness self-test passed")
    return 0


def main() -> int:
    parser = argparse.ArgumentParser(description=__doc__)
    parser.add_argument("--has-supabase-access-token")
    parser.add_argument("--has-supabase-project-id")
    ...
    values = {
        "has_supabase_access_token": args.has_supabase_access_token,
        "has_supabase_project_id": args.has_supabase_project_id,
