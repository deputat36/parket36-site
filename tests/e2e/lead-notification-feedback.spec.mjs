...
test('disabled сообщает, что заявка сохранена, но рекомендует позвонить', async ({ page }) => {
...
test('partial_failure в callback не обещает звонок и передаёт состояние в событие', async ({ page }) => {
