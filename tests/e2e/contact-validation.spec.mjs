...
test('форма не отправляет заявку без пригодного номера для обратного звонка', async ({ page }) => {
  ...
  await page.route(leadEndpoint, async route => {
    ...
    await route.fulfill({
      ...
      body: JSON.stringify({ ok: true, lead_id: 999, notification: 'sent' })
    });
  });
...
test('форма принимает распространённый формат российского телефона', async ({ page }) => {
  ...
  await page.route(leadEndpoint, async route => {
    ...
    await route.fulfill({
      status: 200,
      contentType: 'application/json',
      body: JSON.stringify({ ok: true, request_id: payload.request_id, lead_id: 1000, notification: 'sent' })
    });
  });
...
test('серверная ошибка телефона возвращает фокус в контактное поле', async ({ page }) => {
  let attempts = 0;
  await page.route(leadEndpoint, async route => {
    attempts += 1;
    await route.fulfill({
  ...
  const contact = page.locator('#request-contact');
  await expect(page.locator('#request-status')).toContainText('не менее 10 цифр');
  await expect(page.locator('#request-status')).toContainText('отправьте заявку ещё раз');
  await expect(contact).toHaveAttribute('aria-invalid', 'true');
  await expect(contact).toBeFocused();
