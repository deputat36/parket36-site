...
test('страницы услуг используют общую шапку и mobile CTA, сохраняя тематический футер', async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 });

  for (const profile of [
    {
      path: '/uslugi/parket-i-poly/',
      finalCtaHeading: 'Покажите состояние пола Ивану',
      footerText: 'Паркет, деревянные полы, циклёвка и реставрация.'
    },
    {
    ...
    {
      path: '/uslugi/ukladka-parketa/',
      finalCtaHeading: 'Новый деревянный пол начинается с основания',
      footerText: 'Паркет, деревянные полы, укладка и реставрация.'
    },
    {
      path: '/uslugi/ukladka-laminata/',
      finalCtaHeading: 'Планируете укладку ламината?',
      footerText: 'Паркет, ламинат и деревянные полы.'
    },
    {
    ...
    {
      path: '/uslugi/terrasy-i-derevyannye-poly/',
      finalCtaHeading: 'Терраса посерела и стала шероховатой?',
      footerText: 'Паркет, доска, террасы и деревянные полы.'
    },
    {
      path: '/uslugi/pokrytie-lakom-i-maslom/',
      finalCtaHeading: 'Финиш лучше выбирать после оценки пола',
      footerText: 'Паркет, деревянные полы и защитные покрытия.'
    }
  ]) {
    await page.goto(profile.path);
...
test('страницы решений используют семейный header и mobile CTA без ложной подсветки меню', async ({ page }) => {
  await page.setViewportSize({ width: 390, height: 844 });

  for (const profile of [
    {
      path: '/resheniya/remont-posle-arendatorov/',
      finalCtaHeading: 'Нужно быстро подготовить квартиру к новой сдаче?',
      footerText: 'Пол, паркет и заметные дефекты после арендаторов — без лишнего ремонта ради ремонта.'
    },
    {
      path: '/resheniya/obnovit-pol-posle-arendatorov/',
      finalCtaHeading: 'Подготовьте пол к новой сдаче без лишнего ремонта',
      footerText: 'Паркет, деревянные полы и подготовка пола к новой сдаче.'
    },
    {
      path: '/resheniya/podgotovka-kvartiry-k-prodazhe/',
      finalCtaHeading: 'Хотите подготовить квартиру без лишних расходов?',
      footerText: 'Пол, паркет и заметные дефекты перед продажей квартиры — без ремонта ради ремонта.'
    },
    {
      path: '/resheniya/dlya-rieltorov-i-sobstvennikov/',
      finalCtaHeading: 'Нужно понять, что сделать перед показами?',
      footerText: 'Пол, паркет и заметные дефекты перед продажей, сдачей или заселением.'
    },
    {
      path: '/resheniya/podgotovit-parket-k-prodazhe-kvartiry/',
      finalCtaHeading: 'Покажите пол до начала показов',
      footerText: 'Паркет, подготовка объектов и помощь перед продажей недвижимости.'
    }
  ]) {
    await page.goto(profile.path);
