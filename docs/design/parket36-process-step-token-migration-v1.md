...
На момент миграции `.steps` используется на 86 публичных страницах сайта.
...
Validator проверяет, что на страницах с `HowTo` количество элементов `HowToStep` совпадает с количеством видимых `<li>` в `.steps`.
