...
В нём на новую систему переведены только базовые кнопки `.btn` и их состояния. Остальные production CSS-модули пока продолжают использовать прежние значения и не должны обращаться к `var(--p36-...)`.
...
## Границы миграции кнопок

Изменяются только визуальные свойства:
...
- кнопки соответствуют контракту компонентов;
