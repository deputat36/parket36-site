...
- `design/prototypes/components-v1-process-step.css` — изолированные стили Process Step;
...
Process Step — неинтерактивный элемент нативного `<ol class="steps">`. Номер создаётся CSS-counter, заголовок хранится в `<strong>`, описание — в `<span>`.

На широких экранах используется сетка из трёх колонок с gap 16 px; до 640 px — одна колонка. Минимальная высота этапа — 164 px, номер — 36×36 px, радиус — `radius.lg`.

Порядок `<li>` является смысловым и не должен изменяться визуальными средствами. Ссылки, кнопки, `tabindex`, action-роли, курсор `pointer`, hover-transform и состояния focus/pressed/selected запрещены. На страницах с HowTo schema количество `HowToStep` должно совпадать с количеством видимых этапов.
...
- Process Step в каталоге использует нативный `<ol>` и три неинтерактивных `<li>` без ссылок и обработчиков.
