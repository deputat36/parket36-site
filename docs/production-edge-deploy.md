...
Workflow `.github/workflows/deploy-lead-function.yml` предназначен только для ручного развёртывания двух production-функций после настройки обязательных secrets:
...
5. запускает unit-тесты и `deno check` для обеих функций;
6. получает список имён remote secrets;
7. сверяет `SUPABASE_PROJECT_ID` с endpoint из `data/site.json`;
8. проверяет оба раздела `supabase/config.toml` и обязательные файлы функций;
