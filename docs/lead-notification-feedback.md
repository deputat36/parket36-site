...
### Повторная заявка и `duplicate: true`

При повторе того же `request_id` backend отвечает `duplicate: true` и не создаёт новую запись. Такой ответ не запускает уведомление заново и в текущем контракте не содержит поле `notification`.

Поэтому frontend трактует состояние уведомления как `unknown`, прямо сообщает, что повторная отправка не подтверждает автоматическое уведомление Ивану, и восстанавливает аварийный блок после очистки в начале submit. Пользователь снова получает кнопку звонка, а в подробной форме видит, что текст скопирован повторно.

Это исключает ложное ощущение, что уже сохранённая заявка обязательно была замечена Иваном.
...
При новом submit старый аварийный блок удаляется до следующей попытки. Поэтому повторная отправка не накапливает дублирующиеся кнопки. После ответа `sent` блок исчезает, а после `duplicate: true` без подтверждённого уведомления создаётся заново.
...
7. отдельно обрабатывает `duplicate: true`, не считая его подтверждением уведомления;
8. создаёт отдельное событие `parket36:lead-notification`.
...
5. `duplicate: true` без `notification` восстанавливает аварийный путь после повторной отправки;
6. ошибка backend сохраняет текстовый fallback и показывает звонковое действие;
7. состояния попадают в браузерное событие и `dataLayer`;
8. callback-request получает состояние до своего dispatch.
...
- восстановление fallback для `duplicate: true`;
