...
## Жизненный цикл monitoring issue

Для каждого вида проверки существует не более одного открытого monitoring issue.

При повторной ошибке тело существующего issue обновляется на месте:

- дата `Checked` заменяется актуальной;
- ссылка `Workflow run` указывает на последнюю проверку;
- diagnostic report заменяется последним снимком;
- повторяющиеся комментарии с одинаковой ошибкой не накапливаются.

При восстановлении workflow добавляет один recovery-комментарий и закрывает issue с причиной `completed`. Если ошибка возникнет снова позднее, создаётся новый issue с новым жизненным циклом.

Так заголовок и основное описание всегда показывают последнее фактическое состояние, а история восстановлений остаётся видимой без потока повторных диагностических комментариев.
